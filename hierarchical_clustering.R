...
colnames(data) <- c("query", "subject", "identity", "alignment_length", "mismatches", "gap_opens", "q_start", "q_end", "s_start", "s_end", "evalue", "bit_score")
...
single_clustering <- hclust(dist(dissimilarity_matrix), method = "single")
# coeff <- coef(hierarchical_clustering)
plot(single_clustering, hang = -1, main = "Hierarchical Dendogram")
# cls3 <- cutree(single_clustering, k=4)

#cut the dendogram such that 3 clusters are produced
rect.hclust(single_clustering, k=4, border=2:4)
#fviz_cluster(list(data = single_clustering, cluster = cls3))
dend1 <- as.dendrogram(single_clustering)
de
fviz_nbclust(InputData, FUN = hcut, hc_method = "complete", method = "silhouette", k.max = 10) +
  labs(subtitle = "The Elbow Method")
